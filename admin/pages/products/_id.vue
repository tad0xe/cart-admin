...
export default {
  async asyncData({ $axios, params }) {
    try {
      let categories = $axios.$get('https://taad.herokuapp.com/api/categories')
      let owners = $axios.$get('https://taad.herokuapp.com/api/owners')
      let product = $axios.$get(
        `https://taad.herokuapp.com/api/products/${params.id}`
      )
  ...
  methods: {
    async onUpdateProduct() {
      ...
      const response = await axios
        .put(`https://taad.herokuapp.com/api/products/${this.$route.params.id}`, article)
        .then(() => {
          this.$router.push('/')
        })
    }
